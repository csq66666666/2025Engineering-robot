...
static void DecodeDJIMotor(CANInstance *_instance)
{
    ...
    {
        measure->init_flag = 0;
        measure->init_angle = measure->total_angle;
    }
}
...
void DJIMotorControl()
{
    ...
    DJIMotorInstance *next_motor;
    Motor_Control_Setting_s *motor_setting; // 电机控制参数
    Motor_Controller_s *motor_controller;   // 电机控制器
    DJI_Motor_Measure_s *measure;           // 电机测量值
    float pid_measure, pid_ref;             // 电机PID测量值和设定值
    float pid_measure_delta;
    ...
    { // 减小访存开销,先保存指针引用
        ...
        // 同步调速
        if ((motor->motor_can_instance->can_handle == &hcan2) && ((motor->motor_can_instance->tx_id >= 1) && (motor->motor_can_instance->tx_id <= 4)))
        {
            if (motor->motor_can_instance->tx_id == 1)
                pid_measure_delta = 0.5f * ((dji_motor_instance[i]->measure.total_angle - dji_motor_instance[i]->measure.init_angle) + (dji_motor_instance[i + 1]->measure.total_angle - dji_motor_instance[i + 1]->measure.init_angle));
            else if (motor->motor_can_instance->tx_id == 3)
                pid_measure_delta = 7.0f * ((dji_motor_instance[i]->measure.total_angle - dji_motor_instance[i]->measure.init_angle) + (dji_motor_instance[i + 1]->measure.total_angle - dji_motor_instance[i + 1]->measure.init_angle));

            pid_ref = pid_ref - pid_measure_delta;
        }
